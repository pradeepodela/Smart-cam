...
thres = 0.5
nms_threshold = 0.2
classfile = r'models\coco.names'
configpath = r'models\ssd_mobilenet_v3_large_coco_2020_01_14.pbtxt'
weightspath = r'models\frozen_inference_graph.pb'
...
model = tensorflow.keras.models.load_model(r'models\keras_model.h5')
...
def emergency_vechicledetecation(frame):
    ...
    sup = prediction[0][0].round()
    global amc

    if sup == 1.0:
        amc = amc + 1
    if amc == 10.0:
        print('emergency vechicle detecated')
    ...
    emc_data = {'ambulance':prediction[0][0].round(),'firengine':prediction[0][1].round(),'policecar':prediction[0][2].round()}
   
    text = (f'{ambulance}% {firengine}% {policecar}% {trafic}% {amc}')

   
    return text ,ambulance,firengine
...
def img_p(frame , thres=0.6 , nms_threshold=0.2 , pltbox = False , tracker_plt = False ):
    ...
    if tracker_plt:
        ...
        for box_id in boxes_ids:
            ...
            no_vechicles.append(str(id))
            print('totall number of vechicles on road:'+no_vechicles[-1])
